...
export class AddComponent implements OnInit {
  ...
  enviarDatos(): any {
    ...
    switch (this.opcion) {
      ...
      case "canje": 
          ...
          if (this.idProveedorControl && this.idInsumoControl && this.cantidadInsumooControl
            ...
            if (!idProveedor || !idInsumo || !cantidadInsumo || !idProducto || !cantidadProducto) {
              ...
              const proveedor = this.proveedores.find((p: { nombre: any; }) => p.nombre === idProveedor);
              this.formularioMultiuso.patchValue({IDProveedor: proveedor.IDProveedor});
              console.log(this.formularioMultiuso.get('IDProveedor'))

              const insumo = this.insumos.find((p: {NombreInsumo: any; }) => p.NombreInsumo === idInsumo);
              this.formularioMultiuso.patchValue({IDInsumo: insumo.IDInsumo});
              console.log(this.formularioMultiuso.get('IDInsumo'))

              const producto = this.productos.find((p: {NombreProducto: any; }) => p.NombreProducto === idProducto);
              this.formularioMultiuso.patchValue({IDProducto: producto.IDProducto});
              console.log(this.formularioMultiuso.get('IDProducto'))
              
              const profit= this.Profit(insumo.IDInsumo,producto.IDProducto);
      ...
      this.idProveedorControl = this.formularioMultiuso.get('IDProveedor');
      this.idInsumoControl = this.formularioMultiuso.get('IDInsumo');
      this.cantidadInsumooControl = this.formularioMultiuso.get('CantidadCompra');
